...
// --- DUPLICATED CONSTANTS/LOGIC FROM ORIGINAL main.rs TO ENSURE SAFETY ---
...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header-container { width: 100%; height: 48px; position: relative; padding: 0 15px; overflow: hidden; }
    .drag-area { position: absolute; top: 0; left: 0; width: 100%; height: 48px; z-index: 1; }
    .title { position: absolute; top: 50%; left: 15px; transform: translateY(-50%); z-index: 2; pointer-events: none; white-space: nowrap; line-height: normal; }
    .title img { vertical-align: middle; }
    .title span { font-size: 13px; font-weight: 600; color: #333; margin-left: 10px; vertical-align: middle; }
    .browser-controls { position: absolute; top: 0; left: 220px; height: 48px; z-index: 10; }
    .nav-btn { float: left; width: 32px; height: 32px; margin-top: 8px; margin-right: 4px; text-align: center; line-height: 32px; border-radius: 6px; cursor: pointer; transition: 0.2s; color: #555; }
    ...
    .controls { position: absolute; right: 0; top: 0; height: 48px; z-index: 10; }
    .btn { float: left; width: 48px; height: 100%; text-align: center; line-height: 48px; cursor: pointer; transition: background 0.2s; color: #444; }
...
<body onmousedown="if(event.button === 0 && !event.target.closest('.btn') && !event.target.closest('.nav-btn')) window.ipc.postMessage('drag')">
    <div class="header-container">
        ...
        <div class="title"><!-- LOGO_PLACEHOLDER --><span>Matchapro GC Desktop</span></div>
        <div class="browser-controls">
            <div class="nav-btn" onclick="window.ipc.postMessage('back')" title="Back"><svg viewBox="0 0 24 24"><path d="M20,11V13H8L13.5,18.5L12.08,19.92L4.16,12L12.08,4.08L13.5,5.5L8,11H20Z" /></svg></div>
            <div class="nav-btn" onclick="window.ipc.postMessage('forward')" title="Forward"><svg viewBox="0 0 24 24"><path d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z" /></svg></div>
            <div class="nav-btn" onclick="window.ipc.postMessage('refresh')" title="Refresh"><svg viewBox="0 0 24 24"><path d="M17.65,6.35C16.2,4.9 14.21,4 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20C15.73,20 18.84,17.45 19.73,14H17.65C16.83,16.33 14.61,18 12,18A6,6 0 0,1 6,12A6,6 0 0,1 12,6C13.66,6 15.14,6.69 16.22,7.78L13,11H20V4L17.65,6.35Z" /></svg></div>
        </div>
        <div class="controls">
...
(function() {
    ...
    style.textContent = `
        #matchapro-desktop-toolbar { position: fixed; top: 20px; right: 20px; z-index: 1000000; display: flex; gap: 10px; background: rgba(255, 255, 255, 0.9); padding: 8px 12px; border-radius: 30px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); backdrop-filter: blur(5px); border: 1px solid rgba(0, 0, 0, 0.1); user-select: none; transition: opacity 0.3s; }
        .toolbar-btn { width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; color: #555; transition: all 0.2s; }
    ...
    toolbar.innerHTML = `
        <div class="toolbar-btn" onclick="window.history.back()" title="Back"><svg viewBox="0 0 24 24"><path d="M20,11V13H8L13.5,18.5L12.08,19.92L4.16,12L12.08,4.08L13.5,5.5L8,11H20Z" /></svg></div>
        <div class="toolbar-btn" onclick="window.history.forward()" title="Forward"><svg viewBox="0 0 24 24"><path d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z" /></svg></div>
        <div class="toolbar-btn" onclick="window.location.reload()" title="Refresh"><svg viewBox="0 0 24 24"><path d="M17.65,6.35C16.2,4.9 14.21,4 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20C15.73,20 18.84,17.45 19.73,14H17.65C16.83,16.33 14.61,18 12,18A6,6 0 0,1 6,12A6,6 0 0,1 12,6C13.66,6 15.14,6.69 16.22,7.78L13,11H20V4L17.65,6.35Z" /></svg></div>
    `;
...
(function() {
    ...
    Object.defineProperty(window, 'screen', { get: () => ({ ...originalScreen, width: 390, height: 844, availWidth: 390, availHeight: 844, colorDepth: 24, pixelDepth: 24 }) });
    if (typeof window.ontouchstart === 'undefined') { window.ontouchstart = null; }
    if (typeof navigator.maxTouchPoints === 'undefined' || navigator.maxTouchPoints === 0) { Object.defineProperty(navigator, 'maxTouchPoints', { get: () => 5 }); }
})();
"#;
...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .footer-container { width: 100%; height: 40px; padding: 0 20px; position: relative; }
    .status { position: absolute; left: 20px; top: 0; height: 100%; line-height: 40px; }
    ...
    .credits { position: absolute; right: 20px; top: 0; height: 100%; line-height: 40px; font-weight: 500; }
...
<body>
    <div class="footer-container">
        <div class="status"><div id="dot" class="dot offline"></div><span id="text">Menghubungkan...</span></div>
...
pub async fn run(window: Arc<tao::window::Window>, event_loop: tao::event_loop::EventLoop<crate::UserEvent>, proxy: tao::event_loop::EventLoopProxy<crate::UserEvent>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let header_html = HEADER_HTML.replace("<!-- LOGO_PLACEHOLDER -->", &format!("<img src=\"data:image/png;base64,{}\" style=\"height: 36px; width: auto; vertical-align: middle;\" />", se_black_b64));
    ...
    let header_wv = WebViewBuilder::new(&window)
        ...
        .with_ipc_handler({
            ...
            move |msg| match msg.body().as_str() {
                ...
                "back" => { let _ = proxy_c.send_event(crate::UserEvent::GoBack); }
                "forward" => { let _ = proxy_c.send_event(crate::UserEvent::GoForward); }
                "refresh" => { let _ = proxy_c.send_event(crate::UserEvent::Reload); }
                _ => {}
            }
        })
        .build()?;
        ...
        *control_flow = tao::event_loop::ControlFlow::Wait;
        match event {
            ...
            Event::UserEvent(ev) => match ev {
                crate::UserEvent::UpdateStatus(on) => { let _ = footer_wv.evaluate_script(&format!("if(window.updateStatus)updateStatus({});", on)); }
                crate::UserEvent::UpdateProgress(p) => { let _ = header_wv.evaluate_script(&format!("if(window.updateProgress)updateProgress({});", p)); }
...
fn resize_views(h: &WebView, c: &WebView, f: &WebView, w: u32, h_total: u32, scale_factor: f64) {
    ...
    let _ = h.set_bounds(wry::Rect { position: wry::dpi::Position::Physical(wry::dpi::PhysicalPosition::new(0, 0)), size: wry::dpi::Size::Physical(wry::dpi::PhysicalSize::new(w, header_height)) });
    let main_h = if h_total > header_height + footer_height { h_total - header_height - footer_height } else { 1 };
    let _ = c.set_bounds(wry::Rect { position: wry::dpi::Position::Physical(wry::dpi::PhysicalPosition::new(0, header_height as i32)), size: wry::dpi::Size::Physical(wry::dpi::PhysicalSize::new(w, main_h)) });
    let _ = f.set_bounds(wry::Rect { position: wry::dpi::Position::Physical(wry::dpi::PhysicalPosition::new(0, (header_height + main_h) as i32)), size: wry::dpi::Size::Physical(wry::dpi::PhysicalSize::new(w, footer_height)) });
}
